...
# has_trivial = hasattr(transforms, "TrivialAugmentWide")
# has_auto = hasattr(transforms, "AutoAugment")
...
#     (transforms.TrivialAugmentWide(num_magnitude_bins=31) if has_trivial else None)
#     or (transforms.AutoAugment(transforms.AutoAugmentPolicy.IMAGENET) if has_auto else None),
#     transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.02),
#     transforms.ToTensor(),
#     transforms.Normalize([0.485,0.456,0.406], [0.229,0.224,0.225]),
#     transforms.RandomErasing(p=0.25, scale=(0.02, 0.08), ratio=(0.3, 3.3), inplace=True),
# ]
# train_transform = transforms.Compose([t for t in train_transforms_list if t is not None])
...
def main():
    ...
    valset = ImageFolder(validation_image_path, val_transform)
    ...
    #     num_workers=4 if cuda_ok else 0, pin_memory=cuda_ok, drop_last=True
    # )
    # valloader = torch.utils.data.DataLoader(
    #     valset, batch_size=args.batch_size, shuffle=False,
    #     num_workers=4 if cuda_ok else 0, pin_memory=cuda_ok
    # )
    ####################################
